...
class TestExportOasis(unittest.IsolatedAsyncioTestCase):
    async def test_export_oasis_writes_cache_file_for_etd(self):
        payload = {
            "data": [{"id": "x1"}, {"id": "x2"}],
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                os.chdir(tmpdir)
                with patch("src.export_oasis.requests.get", return_value=response) as mock_get:
                    await export_oasis("https://example.test", col, headers)
            finally:
    ...
    async def test_export_oasis_raises_when_response_is_ko(self):
        response = Mock()
        response.text = "ko!"
        response.raise_for_status.return_value = None
        ...
        with patch("src.export_oasis.requests.get", return_value=response):
            with self.assertRaises(Exception):
                await export_oasis("https://example.test", col, {})

    async def test_export_oasis_raises_when_data_is_empty(self):
        response = Mock()
        response.text = "ok"
        response.raise_for_status.return_value = None
    ...
    async def test_export_oasis_logs_warning_on_http_error(self):
        response = Mock()
        ...
        with patch("src.export_oasis.requests.get", return_value=failing_response):
            with patch("src.export_oasis.logger.warning") as mock_warning:
                await export_oasis("https://example.test", col, {})
